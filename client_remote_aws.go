...
import (
	...
	"github.com/junaozun/rpcx-benchmark/common"
...
func main() {
	...
	for _, serializeType := range allSerializeType {
		...
		for i := 1; i <= 10; i++ {
			...
			f.SetCellValue("Sheet2", common.GetExecTableNum(serializeType)+strconv.Itoa(i+1), tps)
		}
		f.SetCellValue("Sheet2", "A12", "avg")
		f.SetCellValue("Sheet2", common.GetExecTableNum(serializeType)+"12", total/10)
	}
	// 根据指定路径保存文件
